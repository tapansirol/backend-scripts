...
sed 's/{{ NAMESPACE }}/'$namespace'/g' $scriptDir/ibm-ucd-prod-rb.yaml > $scriptDir/$namespace-ibm-ucd-prod-rb.yaml

# Create the role binding for all service accounts in the current namespace
kubectl create -f $scriptDir/$namespace-ibm-ucd-prod-rb.yaml -n $namespace
